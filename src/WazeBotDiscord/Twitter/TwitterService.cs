...
namespace WazeBotDiscord.Twitter
{
    public class TwitterService
    {
        ...
        public TwitterService(DiscordSocketClient client, CancellationToken cancellationToken)
        {
        ...
        public async Task InitTwitterServiceAsync()
        {
            ...
            {
                t.RequiredKeywordsMatch = t.RequiredKeywords.ToWordCheckRegex();
        ...
        async void CheckTwitterAsync(object twitterToCheckIn)
        {
            ...
            if (existing == null)
                tweets = await GetTweetsAsync(twitterToCheck.UserId, twitterToCheck.RequiredKeywordsMatch, null);
            else
                tweets = await GetTweetsAsync(twitterToCheck.UserId, twitterToCheck.RequiredKeywordsMatch, existing.TweetId);
        ...
        async Task<List<Tweet>> GetTweetsAsync(long userId, Regex requiredMatch, long? sinceId)
        {
            ...
            tweets.RemoveAll(t => !requiredMatch.IsMatch(t.Text));
